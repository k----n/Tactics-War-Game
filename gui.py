...
import tiles, unit, powerup, animation, gather
from unit import *
from powerup import *
...
class GUI(LayeredUpdates):
    ...
    def update(self):
        ...
        # See if unit moved is on any power ups
        for p in base_power.BasePower.active_powers:
    ...
    def draw(self):
        ...
        base_power.BasePower.active_powers.draw(self.screen)
    ...
    def draw_bar(self):
        ...
        #Get the hovered power up
        hov_power = powerup.base_power.BasePower.get_power_at_pos(coords)
        
        if hov_power:
            #title for tile section
            self.draw_bar_title("HOVERED POWER UP", line_num)
            line_num += 1
